 ...
 * 图片生成
 */
export default async function generateImages(ctx: Context) {
  const { 
    model = 'cogview-3',
    prompt,
    n = 1,
  ...
  try {
    ...
    logger.info('发送图片生成请求:', { model, prompt, n, size });
    
    // 发送请求到GLM API
    const response = await axios.post(`${GLM_API_BASE}/images/generations`, requestData, {
      headers: {
        'Content-Type': 'application/json',
    ...
    if (response.data.error) {
      ...
      throw new APIException(EX.API_REQUEST_FAILED, response.data.error.message || '图片生成失败');
    }
    ...
    logger.info('图片生成成功:', { count: result.data.length });
    ...
    if (error.code === 'ECONNABORTED') {
      throw new APIException(EX.API_REQUEST_FAILED, '图片生成超时，请稍后重试');
    }
    
    throw new APIException(EX.API_REQUEST_FAILED, '图片生成服务暂时不可用');
  }
}